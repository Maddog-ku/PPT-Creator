...
使用者選定模板後，系統會產生可再次編輯的 PowerPoint 檔案（`.pptx`），也可以輸出 PDF。生成完成的簡報會先保存在平台中，只有在使用者主動按下「下載」後，檔案才會傳送到使用者裝置。
...
3. 使用者同時輸入提示文字與參考文件，例如指定受眾、語氣、頁數及重點。
...
│   ├── api/                  # FastAPI 後端
│   └── renderer/             # PPTX / PDF 渲染服務
├── packages/
│   ├── presentation-schema/  # 簡報 JSON Schema 與共用型別
│   ├── template-sdk/         # 模板定義與驗證工具
│   └── shared/               # 共用常數與工具
├── templates/                # 內建模板與預覽資源
├── storage/                  # 開發環境檔案資料（不提交 Git）
├── docs/                     # 產品、API、架構與設計文件
├── docker-compose.yml
├── .env.example
└── README.md
